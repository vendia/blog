...
For example, consider Organization A that adopts an event-driven microservices architecture for core systems and Organization B that still relies on error-prone batch processes.  The value of Organization A's investment is limited as soon as it attempts to collaborate with Organization B.  Whether the interaction among those organizations takes the form of periodic SFTP transfers (Organization B's preference) or point-to-point API interactions (Organization A's preference), neither duplicates the real-time, event-driven interactions from a shared source of truth that Organization A aspired to create in-house.

As [partner ecosystems become central](https://www.bearingpointbeyond.com/en/insights/market-insights/partner-ecosystem/) to unlocking new business opportunities, it's imperative that Organizations, no matter their real-time data sharing maturity, have an easy path toward joint real-time, event-driven solutions.  Further, it's equally important to allow each Organization to make the most of its substantial investment in its in-house real-time data sharing capabilities.  For example, collaboration between Organization A and Organization B should be able to leverage the substantial investment Organization A made to modernize its in-house systems.  More often than not, this means not only working seamlessly across organizational boundaries but also seamlessly across Cloud Service Providers (CSPs), as it's common for partners to adopt different CSPs while still wanting to work effectively together.
...
* Login to your Azure environment using the Azure CLI
    * `az login`
* Identify the Subscription ID of your [Azure Subscription](https://docs.microsoft.com/en-us/azure/active-directory/fundamentals/active-directory-how-subscriptions-associated-directory) in which your Azure environment resides and set the Azure Subscription against which future Azure CLI commands will execute
...
* Open the Vendia Share web site ([https://share.vendia.net/uni](https://share.vendia.net/uni)), and click your Uni’s name. Find the Retailer Node and click on its `GraphQL Explorer` button. Clear the sample query in the middle pane, and insert the mutation below.
  <details>
