...
Let's quickly review what makes an app _lean_. A lean app [flips the IT iceberg](https://www.vendia.net/blog/lean-app-part-4#the-lean-app-goal-flip-the-it-iceberg) by allowing development teams to _focus primarily _on tasks that result in differentiated business value. The goal is to maximize the value and innovation a team produces by drastically reducing the time spent on undifferentiated, repetitive tasks that aren't material to the business. Those high-toil tasks are certainly necessary for a working system, but ideally, those tasks are offloaded through automation and platform capabilities so that (highly creative, competent, and compensated) people) don't have to spend much time on them.
...
* Developers have ready access to countless REST and GraphQL libraries and frameworks used to create APIs from scratch. But is API _implementation _a high-value activity that developers should focus on?
...
## While these are just some examples focused on API solutions, they highlight the central guiding principle of lean apps: Less is more. The benefits of lean apps for teams
...
**Want to go deeper and explore more about lean apps?**



* [The lean app movement](https://www.vendia.net/blog/lean-app)
* [Lean apps, part  1: Lessons from the past](https://www.vendia.net/blog/lean-app-part-1)
* [Lean apps, part 2: The elements of a lean app](https://www.vendia.net/blog/lean-app-part-2)
* [Lean apps, part 3: Let infrastructure do the dirty work](https://www.vendia.net/blog/lean-app-part-3)
* [Lean apps, part 4: The lean app manifesto](https://www.vendia.net/blog/lean-app-part-4)
